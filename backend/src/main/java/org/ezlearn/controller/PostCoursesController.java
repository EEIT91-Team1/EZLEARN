...
//讀取課程介紹回網頁
	...
	@PostMapping("/savecoursesummory")
	public ResponseEntity<String> saveCourseSummory(@RequestBody CourseInformation courseInformation){
		String courseSummory = courseInformation.getCourseSummory();
		postCoursesService.saveCourseSummory(courseSummory); // 呼叫 Service 處理邏輯
		return ResponseEntity.ok("課程名稱已成功暫存：" + courseSummory);
	}
//讀取課程總覽回網頁
	@PostMapping("/loadcoursesummory")
	public ResponseEntity<Map<String, Object>> loadCourseSummory() {
		Map<String, Object> response = new HashMap<>();
		String courseSummory = postCoursesService.getCourseSummary();
		if (courseSummory != null) {
			response.put("data", courseSummory);
		} else {
			response.put("data", null);  // 空資料
		}
		return ResponseEntity.ok(response);  // 返回 200 OK，資料即使為空
